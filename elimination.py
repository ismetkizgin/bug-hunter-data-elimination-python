...
elemination_type = 'random'  # head - random
...
def delete_rows(data, count, delete_type):
    if (delete_type == 'random'):
        ...
        data_filtered = data[data['Number of Bugs'] == 0].head(count)

        data = data.drop(data_filtered.index, axis=0).reset_index(drop=True)
        return data
...
for data_name in data_name_list:
    ...
    count_of_rows_delete = len(data_filtered_nob_0) - len(data_filtered_nob_1) * 2

    if(count_of_rows_delete < -1):
        count_of_rows_delete = 0

    if(str(data_name) == 'all'):
        data = delete_rows(data, 30000, elemination_type)
    else:
        data = delete_rows(data, count_of_rows_delete, elemination_type)
